...
# numberis the sum of the two previous numbers. This series appears
...
# consideringthat it takes 6 iterations before it generates the first 2
# digit number.
...
# check if number is Fibonacci: (5 * (n**2)) + 4 || (5 * (n**2)) - 4
...
def find_fibonacci_index_by_length(length)
  ...
  n = 0
  loop do
    fib_number = fib_array[n] + fib_array[n + 1]
    fib_array << fib_number
    n += 1
    return fib_array.index(fib_number) + 1 if fib_number >= limit
  end
end
